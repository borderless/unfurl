...
var CatboxFs = require('catbox-fs')
var scrappy = require('../')
...
describe('scrappy', function () {
  const cache = popsicleCache.plugin({
    engine: new CatboxFs({ directory: path.join(__dirname, '__cache__') }),
    cacheable: popsicleCache.cacheables.always(),
    ttl: popsicleCache.ttls.forever(),
    serializer: popsicleCache.serializers.stream(20 * 1000 * 1000),
    handler: popsicleCache.handlers.always(),
    waitForCache: true
  })
